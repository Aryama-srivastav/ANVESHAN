...
CREATE OR REPLACE FUNCTION anveshan_is_admin() RETURNS BOOLEAN LANGUAGE sql STABLE AS $$
  SELECT EXISTS (
    ...
    WHERE ur.user_id = current_setting('app.current_user_id', true)::uuid AND r.name = 'admin'
  );
$$;

CREATE OR REPLACE FUNCTION anveshan_has_case_access(target_case UUID, required_level TEXT DEFAULT 'read') RETURNS BOOLEAN LANGUAGE sql STABLE AS $$
...
CREATE POLICY grants_access ON authorized_access USING (user_id = current_setting('app.current_user_id', true)::uuid OR anveshan_is_admin()) WITH CHECK (anveshan_is_admin());
CREATE POLICY signatures_access ON document_signatures USING (EXISTS (SELECT 1 FROM document_versions v JOIN documents d ON d.id = v.document_id WHERE v.id = document_version_id AND anveshan_has_case_access(d.case_id))) WITH CHECK (TRUE);
CREATE POLICY ledger_access ON audit_ledger_records USING (anveshan_is_admin() OR (case_id IS NOT NULL AND anveshan_has_case_access(case_id))) WITH CHECK (TRUE);
...
DROP FUNCTION IF EXISTS anveshan_has_case_access;
